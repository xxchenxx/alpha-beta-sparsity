...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = resnet.resnet18(pretrained=False, num_classes=100)
    ...
    model.new_fc = nn.Linear(512, 1000)
    ...
    if args.distributed:
        ...
        train_imagenet_sampler = torch.utils.data.distributed.DistributedSampler(imagenet_train_dataset)

    else:
        train_sampler = None
        train_imagenet_sampler = None
    ...
    train_imagenet_loader = torch.utils.data.DataLoader(
        imagenet_train_dataset, batch_size=args.batch_size, shuffle=False,
        num_workers=args.workers, pin_memory=True, sampler=train_imagenet_sampler)
    ...
    for state in range(start_state, args.pruning_times):
        ...
        for epoch in range(start_epoch, args.epochs):
            if args.distributed:
                ...
                train_imagenet_sampler.set_epoch(epoch)
            print(optimizer.state_dict()['param_groups'][0]['lr'])

            acc = train_mix(train_loader, train_imagenet_loader, model, criterion, optimizer, epoch, args)
