...
parser.add_argument('--n-ways', type=int, default=20)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    train_data, train_label = pickle.load(
        open(os.path.join(args.data, 'processed/train.pkl'), 'rb')
    )
    test_data, test_label = pickle.load(
        open(os.path.join(args.data, 'processed/test.pkl'), 'rb')
    )
    query_shots = 15
    shot = 5
    all_images = np.concatenate([train_data, test_data], 0)
    all_labels = np.concatenate([train_label, test_label], 0)
    ...
    samples_classes = np.random.choice(a=all_classes, size=args.n_ways,replace=False)
    support_samples = []
    query_samples = []
    ...
    y_support = torch.arange(args.n_ways)[:, None].repeat(1, shot).reshape(-1)
    y_query = torch.arange(args.n_ways)[:, None].repeat(1, query_shots).reshape(-1)

    z_support = torch.cat(support_samples, 0)
    z_query = torch.cat(query_samples, 0)

    train_dataset = CustomTensorDataset(
        [y_support, z_support], transform=transforms.Compose(train_transform_list)
    )

    val_dataset = CustomTensorDataset(
        [y_query, z_query], transform=transforms.Compose(test_transforms_list)
    )
