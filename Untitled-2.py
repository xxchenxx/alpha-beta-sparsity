...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.checkpoint and not args.resume:
        ...
        load_state_dict = {}
        start_state = checkpoints[0]['state'] # currently using the sames
        if start_state:
            current_masks = [extract_mask(checkpoint['state_dict']) for checkpoint in checkpoints]

            new_masks = {}
            for key in current_masks[0]:
                new_masks[key] = (torch.sum([c[key] for c in current_masks]) >= 2).long()

            prune_model_custom(model, new_masks, conv1=True)
            check_sparsity(model)
        for name in state_dicts[0]:
             if name.startswith("module."):
                   load_state_dict[name[7:]] = torch.mean([s[name] for s in state_dicts])
             else:
                   load_state_dict[name] = torch.mean([s[name] for s in state_dicts])
